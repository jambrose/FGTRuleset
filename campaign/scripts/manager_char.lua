...
function calcItemArmorClass(nodeChar)
	...
	for _,vNode in pairs(DB.getChildren(nodeChar, "inventorylist")) do
		if DB.getValue(vNode, "carried", 0) == 2 then
			...
			if bIsArmor then
				local bID = ItemManager.getIDState(vNode, true);
...
function addTraitDB(nodeChar, sClass, sRecord)
	...
	local sTraitType = nodeSource.getName();
	...
	if sTraitType == "abilityscoreincrease" then
		...
		if sAdjust:match("your ability scores each increase") then
			...
			if sAdjust:match("two other ability scores of your choice increase") then
				local aAbilities = {};
				...
				wSelect.requestSelection(sTitle, sMessage, aAbilities, CharManager.onRaceAbilitySelect, nodeChar);
			end
		end
...
function pickSkills(nodeChar, aSkills, nPicks, nProf)
	...
	wSelect.requestSelection (sTitle, sMessage, aSkills, CharManager.onClassSkillSelect, rSkillAdd);
end
...
function addBackgroundRef(nodeChar, sClass, sRecord)
	...
	if sSkills ~= "" and sSkills ~= "None" then
		local nPicks = 0;
		local aPickSkills = {};
		if sSkills:match("Choose two from among ") then
			local sPickSkills = sSkills:match("Choose two from among (.*)");
...
function addRaceRef(nodeChar, sClass, sRecord)
	...
	if sClass == "reference_race" then
		...
		elseif #aSelection == 1 then
			...
			local sMessage = string.format(Interface.getString("char_build_message_selectsubrace"), DB.getValue(nodeSource, "name", ""));
...
function addClassRef(nodeChar, sClass, sRecord)
	...
	elseif #aOptions == 1 then
		...
		local sMessage = string.format(Interface.getString("char_build_message_selectspecialization"), DB.getValue(nodeSpecializationFeature, "name", ""));
